...
# Este código SQL consulta la base de datos Northwind para obtener información sobre productos, pedidos, clientes, empleados y proveedores, utilizando diferentes tipos de JOIN para combinar tablas y obtener datos como ventas, stock, y pedidos, aplicando filtros por fechas, categorías y ubicaciones.
...
select concat(e.FirstName, ' ', e.LastName) as [Nombre]
,count(OrderID) as [N�mero de Ordenes]
from Orders as o
inner join Employees as e
on o.EmployeeID = e.EmployeeID
group by e.FirstName, e.LastName
